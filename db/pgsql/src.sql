...
CREATE TABLE IF NOT EXISTS src_scan_ip (
  ...
  CONSTRAINT parent_ipid_ssi FOREIGN KEY (parent_ipid) REFERENCES src_scan_ip (ipid) ON DELETE SET NULL ON UPDATE CASCADE 
);
CREATE INDEX ON src_scan_ip (refid);
CREATE INDEX ON src_scan_ip (sourceid);
CREATE INDEX ON src_scan_ip (mac);
CREATE INDEX ON src_scan_ip (ispublic);
CREATE INDEX ON src_scan_ip (parent_ipid);
...
CREATE UNIQUE INDEX ON src_snmp (ipid, oid, oid_index);
...
CREATE TABLE IF NOT EXISTS src_snmp_vlan (
  ...
  switchid BIGINT NOT NULL,
  vlan INT NOT NULL CHECK (vlan>=0), 
